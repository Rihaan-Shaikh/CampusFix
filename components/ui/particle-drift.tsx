...
export function ParticleDrift(props: ParticleDriftProps) {
  const { resolvedTheme } = useTheme();
  const isDark = resolvedTheme === "dark";

  // Automatic semantic CampusFix palette assignment
  const defaultBg = isDark ? "#080D16" : "#F7F8FA";
  const defaultBase = isDark ? "#64748B" : "#94A3B8";
  const defaultAccent = "#F59E0B"; // CampusFix Brand Orange
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn("ParticleDrift: WebGL unavailable");
      return;
    }
    ...
    const render = (now: number) => {
      ...
      const ca = parseColor(v.accent as string, [0.96, 0.62, 0.04]); // CampusFix orange
